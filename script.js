...
const productDetails = {
    'Z-DermAspis': {
        ...
        bibliography: `<ol><li>World Health Organization (WHO). (2009). WHO Guidelines on Hand Hygiene in Health Care.</li></ol>`
    },
    'ZplastCream 40gr': {
        ...
        bibliography: `<ol><li>Öztürk, N., et al. (2007). Effects of Hypericum perforatum extract on skin wounds...</li></ol>`
    },
    'ZplastCream 100gr': {
        consumer: `<h3>Εξειδικευμένη Αναπλαστική Κρέμα (100gr)</h3><p>Η Zplast Cream είναι σχεδιασμένη για την εντατική φροντίδα και ταχεία επούλωση... Η μεγαλύτερη συσκευασία είναι ιδανική για εκτεταμένη χρήση.</p>`,
        science: `<h3>Αποδεδειγμένη Δράση Συστατικών</h3><p><strong>Βαλσαμόχορτο:</strong> Επιταχύνει την επιθηλιοποίηση...</p>`,
        bibliography: `<ol><li>Öztürk, N., et al. (2007). Effects of Hypericum perforatum extract on skin wounds...</li></ol>`
    },
    'Bruise Off Bite Out & Pain Free cream': {
        ...
        bibliography: `<ol><li>Lyss, G., et al. (1998). Helenalin, an anti-inflammatory sesquiterpene lactone...</li></ol>`
    },
    'Z-boost 30 caps': {
        consumer: `<h3>Ενίσχυση του Ανοσοποιητικού Συστήματος</h3><p>Το Zarkolia Z-Boost είναι ένα προηγμένο συμπλήρωμα διατροφής για την ολιστική ενίσχυση και θωράκιση του ανοσοποιητικού...</p>`,
    ...
    'Z-boost 12 caps': {
        consumer: `<h3>Ενίσχυση του Ανοσοποιητικού Συστήματος</h3><p>Το Zarkolia Z-Boost είναι ένα προηγμένο συμπλήρωμα διατροφής... Η συσκευασία των 12 καψουλών είναι ιδανική για περιοδική ή βραχυπρόθεσμη ενίσχυση.</p>`,
        science: `<h3>Αποδεδειγμένη Δράση Συστατικών</h3><p><strong>Ψευδάργυρος (Zinc):</strong> Απαραίτητο για τη λειτουργία των κυττάρων του ανοσοποιητικού...</p>`,
        bibliography: `<ol><li>Hemilä, H. (2017). Zinc lozenges and the common cold...</li></ol>`
    },
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    products.forEach(p => { const button = document.createElement('button'); button.className = 'product-btn'; button.textContent = p.name; button.onclick = () => showProductDetails(p.name); productButtonsContainer.appendChild(button); });
...
function generateTextContent(isForEmail) {
    const { customerData, items, totals } = getOrderData();
    const bankInfo = `\n\nΣΤΟΙΧΕΙΑ ΚΑΤΑΘΕΣΗΣ\n----------------------------------------\nIBAN: GR8901722520005252016160277\nΤράπεζα: Τράπεζα Πειραιώς\n`;
    
    let content = "========================================\n      Π Α Ρ Α Γ Γ Ε Λ Ι Α\n========================================\n\n";
    
    // Χρησιμοποιούμε ΚΕΦΑΛΑΙΑ και asterisks για έμφαση, αντί για bold
    content += "****************************************\n";
    content += "* ΣΤΟΙΧΕΙΑ ΠΕΛΑΤΗ           *\n";
    content += "****************************************\n";
    content += `ΕΠΩΝΥΜΙΑ: ${customerData.eponimia || '-'}\n`;
    content += `ΑΦΜ:      ${customerData.afm || '-'}\n`;
    content += `ΔΟΥ:      ${customerData.doy || '-'}\n`;
    content += `ΚΙΝΗΤΟ:   ${customerData.mobile || '-'}\n`;
    content += `ΣΤΑΘΕΡΟ:  ${customerData.phone || '-'}\n`;
    ...
    items.forEach(item => {
        ...
        if (isForEmail) {
            // Πιο σύντομη μορφή για το email
            content += `- ${item.name} (Ποσ: ${quantityText}) - Σύνολο: ${item.total}\n`;
        } else {
            // Πιο αναλυτική μορφή για το TXT
            content += `${item.name}\n`;
            content += `  - Ποσότητα:         ${quantityText}\n`;
    ...
    content += "\n\nΣΥΝΟΛΑ\n----------------------------------------\n";
    content += `Καθαρή Αξία:      ${totals.net}\n`;
    content += `Αξία ΦΠΑ (24%):    ${totals.vat}\n`;
    content += `ΤΕΛΙΚΟ ΠΟΣΟ:      ${totals.final}\n`;
...
function sendEmailViaClient() {
    ...
    const body = generateTextContent(true); // true = δημιουργία κειμένου για email
...
function previewAndSaveAsTXT(){
    ...
    const content = generateTextContent(false); // false = δημιουργία κειμένου για TXT

// ============================================================
// ZARKOLIA HEALTH - CORE ENGINE v48.0
// ============================================================
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof products !== 'undefined') {
        products.forEach((p, index) => {
            ...
            row.innerHTML = `
                <td style="padding:20px;"><strong>${p.name}</strong><br><small style="color:#64748b;">${p.price.toFixed(2)} €</small></td>
                <td style="padding:20px; display:flex; justify-content:center;">
                    <div class="stepper" style="display:flex; align-items:center; background:#f1f5f9; padding:5px; border-radius:12px;">
                        <button type="button" onclick="changeQty(${index}, -1)" style="width:32px; height:32px; border-radius:8px; border:none; background:#fff; cursor:pointer; font-weight:800;">−</button>
...
function showInfo(name, index) {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="border-radius:40px; padding:50px;">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:25px;">
                <div style="background:#ecfdf5; padding:30px; border-radius:24px;">
                    <strong style="color:var(--primary); text-transform:uppercase;">📍 Ενδείξεις</strong><br>${p.cases}
                </div>
                <div style="background:#f0f9ff; padding:30px; border-radius:24px;">
                    <strong style="color:#0369a1; text-transform:uppercase;">💡 Rationale</strong><br>${p.rationale || "Zarkolia Exclusive Formulation"}
                </div>
            </div>
        </div>`;
...
async function processOrder() {
    ...
    if (!epo || !afm) { alert("Συμπληρώστε τα στοιχεία πελάτη!"); return; }
    ...
    if (sheetItems.length === 0) { alert("Η παραγγελία είναι άδεια!"); return; }
    ...
    try {
        ...
        const mailBody = `ΑΝΤΙΓΡΑΦΟ ΠΑΡΑΓΓΕΛΙΑΣ%0D%0A%0D%0AΠΕΛΑΤΗΣ: ${epo}%0D%0AΑΦΜ: ${afm}%0D%0A%0D%0AΠΡΟΪΟΝΤΑ:%0D%0A${emailItems}%0D%0A%0D%0AΣΥΝΟΛΟ ΜΕ ΦΠΑ: ${totalVal}%0D%0AΠΑΡΑΤΗΡΗΣΕΙΣ: ${remarks}`;

...
function showProductDetails(productIndex){
    ...
    if(p){
        ...
        modalProductName.textContent = products[productIndex].name;
        consumerTab.innerHTML = `<p>Δεν βρέθηκε αναλυτική περιγραφή.</p>`;
        scienceTab.innerHTML = '';
        biblioTab.innerHTML = '';
    }

    // Σύνδεση του κουμπιού "Ενημέρωση" στο Modal με το πεδίο του πίνακα
    const addButton = document.getElementById('modal-add-button');
    // Παίρνουμε την τρέχουσα τιμή από τον πίνακα
    const tableInput = document.getElementById(`qty-${productIndex}`);
    document.getElementById('modalQuantity').value = tableInput.value;

    // Clone για καθαρισμό event listeners
    const newAddButton = addButton.cloneNode(true);
    addButton.parentNode.replaceChild(newAddButton, addButton);
    
    newAddButton.onclick = () => {
        const quantity = parseInt(document.getElementById('modalQuantity').value) || 0;
        tableInput.value = quantity; // Ενημέρωση του πίνακα
        updateAll(); // Ενημέρωση συνόλων
        closeProductModal();
    };

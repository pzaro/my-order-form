...
const knownCustomers = {
    ...
    "079214571": { eponimia: "ΜΑΝΔΑΛΤΣΗ ΑΙΚΑΤΕΡΙΝΗ ΔΗΜΗΤΡΙΟ", doy: "ΓΙΑΝΝΙΤΣΩΝ", mobile: "69422690321", phone: "2382022735", email: "farmakiomandaltsi@gmail.com" },
...
function showInfo(name) {
    ...
    modal.innerHTML = `<div class="modal-content"><span style="position:absolute;top:20px;right:20px;cursor:pointer;font-size:2.5rem;color:#aaa;" onclick="this.parentElement.parentElement.style.display='none'">&times;</span><h2 style="color:var(--emerald-dark);">${name}</h2>${p.description.consumer}<hr style="margin:20px 0; border:0; border-top:1px solid #eee;">${p.description.science}${p.description.bibliography}</div>`;
    modal.style.display = 'block';
}

// --- ΟΡΙΣΤΙΚΗ ΣΥΝΑΡΤΗΣΗ PROCESS ORDER (CLOUD & EMAIL) ---
async function processOrder() {
    ...
    submitBtn.textContent = "Αποθήκευση στο Cloud...";
